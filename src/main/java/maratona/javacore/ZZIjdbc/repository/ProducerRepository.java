...
public class ProducerRepository {
    ...
    public static void saveTransaction(List<Producer> producers) {
        try (Connection conn = ConnectionFactory.getConnection()) {
            ...
            log.error("Error while trying to save producers '{}' in the database", producers, e);
        }
    }
    ...
    private static void PreparedStatementSaveTransaction(Connection conn, List<Producer> producers) throws SQLException {
        ...
        for(Producer p: producers){
            try (PreparedStatement ps = conn.prepareStatement(sql)){
                ...
                //if (p.getName().equals("White Fox")) throw new SQLException("Error trying to save producer 'White Fox'");
                ps.execute();
            }catch (SQLException e){
                e.printStackTrace();
                shouldRollback = true;
